...
export default function AdminBlogEdit() {
    ...
    async function load() {
      try {
        ...
        if (tagsRes.ok) {
          ...
          setAvailableTags(Array.isArray(d) ? d : d.tags || []);
        }
        if (postRes.ok) {
          ...
          setSelectedTags((post.postTags || []).map((pt) => pt.tag?.name).filter(Boolean));
        } else {
  ...
  async function deletePost() {
    try {
      const r = await fetch(`/api/admin/blogs/${id}`, { method: 'DELETE' });
      if (r.ok) {
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[#888] text-sm">Update blog post content and settings</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Update Post'}
          </Button>
        </div>
      </div>
